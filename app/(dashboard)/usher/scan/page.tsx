...
import { checkInGuest, getGuestByCode, getEventDetail } from "./actions"; 
...
function ScannerContent() {
    ...
    // Helper Variable untuk Logic Tampilan Over Capacity
    const maxAllocated = activeGuest?.totalPaxAllocated || 1;
    const isOverCapacity = inputActualPax > maxAllocated;
            ...
            {!activeGuest ? (
                ...
                <Card className="w-full max-w-md bg-slate-800 border-slate-600 shadow-2xl animate-in zoom-in duration-300">
                    ...
                    <CardContent className="p-6 space-y-8">
                        ...
                        {/* Input Jumlah - DENGAN LOGIKA VISUAL WARNING */}
                        <div className={`p-6 rounded-xl border space-y-4 transition-colors ${
                            isOverCapacity 
                            ? 'bg-amber-900/20 border-amber-500/50' 
                            : 'bg-slate-900/50 border-slate-700/50'
                        }`}>
                            <Label className="text-slate-400 text-xs uppercase tracking-widest text-center block">
                                Jumlah Tamu Masuk (Jatah: {maxAllocated})
                            </Label>
                            <div className="flex items-center justify-center gap-6">
                                ...
                                <div className="text-center w-16">
                                    <span className={`text-5xl font-bold ${isOverCapacity ? 'text-amber-500' : 'text-white'}`}>
                                        {inputActualPax}
                                    </span>
                                </div>
                                <Button 
                            ...
                            {/* ALERT OVER CAPACITY */}
                            {isOverCapacity && (
                                <p className="text-center text-xs text-amber-500 font-bold animate-pulse">
                                    ⚠️ Melebihi Jatah Undangan (+{inputActualPax - maxAllocated})
                                </p>
                            )}
                        </div>
