...
export default async function UsherDashboard() {
  ...
  const activeProjects = await prisma.project.findMany({
      where: { 
          isActive: true,
          activeModules: {
              has: ServiceModule.RSVP_VENUE_SYSTEM
          }
      },
      select: {
          id: true,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {activeProjects.length === 0 ? (
                ...
                activeProjects.map((project) => {
                    ...
                    const eventTime = meta.eventTime || "Waktu Belum Diatur";
                    const location = meta.location || "Lokasi Belum Diatur";
                    const displayDate = meta.eventDate 
                        ? new Date(meta.eventDate).toLocaleDateString('id-ID', { dateStyle: 'full' }) 
                        : "Tanggal Belum Diatur";

                    return (
                        <Card key={project.id} className="bg-slate-800 border-slate-700 text-slate-200 overflow-hidden flex flex-col">
                            ...
                            <CardContent className="pt-4 flex-1 flex flex-col gap-4">
                                <div className="text-sm text-slate-400 space-y-2 flex-1">
                                    <p className="flex items-center gap-2"><Clock className="w-4 h-4 text-slate-600"/> {eventTime}</p>
                                    <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-slate-600"/> {location}</p>
                                </div>
                                
                                <div className="grid grid-cols-2 gap-3 mt-2">
                                    <Link href={`/usher/scan?id=${project.id}`}>
                                        <Button className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold">
                                            <QrCode className="w-4 h-4 mr-2" />
                                            Scan Tamu
                                        </Button>
                                    </Link>
                                    <Link href={`/dashboard/live?id=${project.id}`}>
